...
export const createAlumno = async (req: Request, res: Response) => {
    ...
    try {
        // Generar username: 1a letra nombre + 3 de apellido1 + 3 de apellido2
        const p1 = nombre.charAt(0);
        const p2 = apellido1.substring(0, 3);
        const p3 = apellido2 ? apellido2.substring(0, 3) : '';
        const username = `${p1}${p2}${p3}`.toLowerCase();
