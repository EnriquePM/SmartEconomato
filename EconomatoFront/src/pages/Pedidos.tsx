...
const Pedidos = () => {
    ...
    const [pedidos, setPedidos] = useState<Pedido[]>([
        ...
        },
        {
            id: 'PED-002',
            tipo: 'utensilios',
            proveedor: 'Menaje Profesional SL',
            fecha: '2023-10-26',
            estado: 'Borrador',
            total: 45.00,
            observaciones: 'Reponer sartenes',
            lineas: []
        }
    ]);
    ...
    // --- LÓGICA ---
    const catalogoProductos = tipoPedido === 'productos' ? PRODUCTOS_MOCK : UTENSILIOS_MOCK;
    const catalogoProveedores = tipoPedido === 'productos' ? PROVEEDORES_MOCK : PROVEEDORES_UTENSILIOS_MOCK;
    ...
    const seleccionarProducto = (lineaId: number, prodId: string) => {
        const producto = catalogoProductos.find(p => p.id === parseInt(prodId));
        if (!producto) return;
    ...
    if (vista === 'formulario') {
        ...
        return (
            <div className="flex flex-col animate-fade-in-up space-y-6 pb-6">
                ...
                {/* --- NUEVA CABECERA REESTRUCTURADA (Propuesta 1) --- */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 shrink-0 gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="h-8 w-[1px] bg-gray-200 mx-2"></div> {/* Separador vertical */}
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        {!esSoloLectura && (
                            <div className="flex items-center bg-gray-100 p-1.5 rounded-pill shadow-inner w-full md:w-auto border border-gray-200/50">
                                <button
                                    ...
                                    className="px-6 py-2.5 text-gray-600 rounded-pill font-bold text-xs hover:text-black transition-all duration-200 flex-1 md:flex-none uppercase tracking-wider"
                                >
                                    Guardar Borrador
                                </button>
                                <button
                                    onClick={enviarPedido}
                                    className="px-8 py-2.5 bg-black text-white rounded-pill font-bold text-xs hover:bg-gray-800 transition-all duration-200 shadow-md active:scale-95 flex-1 md:flex-none uppercase tracking-wider"
                                >
                                    Enviar Pedido
                                </button>
                            </div>
                        ...
                        {esSoloLectura && (
                            <span className="text-sm font-bold text-gray-400 italic pr-4">Pedido finalizado (Modo lectura)</span>
                        )}
                    </div>
                </div>
                ...
                {/* --- TARJETA 2: LISTA DE PRODUCTOS --- */}
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div className="p-6 border-b border-gray-100">
                        <h3 className="text-lg font-bold text-gray-800">Lista de {titulo}</h3>
                    </div>
                    ...
                    <div className="overflow-x-auto scrollbar-global p-1">
                                ...
                                {pedidoActual.lineas.map((linea) => (
                                    <tr key={linea.id} className="hover:bg-gray-50 group transition-colors duration-150">
                                        ...
                                        <td className="p-3 align-middle">
                                            <span className="text-sm text-gray-500 bg-gray-100 px-2 py-1 rounded">{linea.categoria || '-'}</span>
                                        </td>
                                        <td className="p-3 w-32 align-middle">
                                            <Input
                                                ...
                                                onChange={(val) => actualizarLinea(linea.id, 'cantidad', val)}
                                            />
                                        </td>
                                        <td className="p-3 text-center text-sm text-gray-500 align-middle">
                                            {linea.unidad || '-'}
                                        </td>
                                        <td className="p-3 w-32 align-middle">
                                            <Input
                                                ...
                                                onChange={(val) => actualizarLinea(linea.id, 'precio', val)}
                                            />
                                        </td>
                                        <td className="p-3 text-right font-bold text-gray-800 align-middle">
                                        ...
                                        <td className="p-3 text-center align-middle">
                                            {!esSoloLectura && (
                                                <button
                                                    ...
                                                    className="text-gray-300 hover:text-red-500 transition-colors font-bold text-xl p-2 rounded-full hover:bg-red-50"
                                                >
                        ...
                        {pedidoActual.lineas.length === 0 && (
                            <div className="p-10 text-center text-gray-400">
                                No hay {titulo.toLowerCase()} en el pedido.
                            </div>
                        )}
                    </div>
                    ...
                    {!esSoloLectura && (
                        <div className="p-4 border-t border-gray-100 bg-gray-50">
                            <button
                                ...
                                className="w-full py-3 bg-white text-gray-600 font-bold rounded-xl hover:bg-gray-50 transition border border-dashed border-gray-300 hover:border-gray-400 flex items-center justify-center gap-2"
                            >
    ...
    return (
        <div className="h-full flex flex-col animate-fade-in-up">
            ...
            <div className="flex gap-2 mt-8 pl-2 relative items-end">
                ...
                <button
                    onClick={() => setTipoPedido('productos')}
                ...
                <button
                    onClick={() => setTipoPedido('utensilios')}
                    className={`px-8 py-4 rounded-t-[1.5rem] text-sm font-bold transition-all relative z-10 border-t border-l border-r ${
                            ...
                            {pedidosFiltrados.length > 0 ? (
                                    ...
                                    <tr key={pedido.id} className="hover:bg-gray-50 transition group">
                                        ...
                                        <td className="p-5 text-center">
                                            <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                                                pedido.estado === 'Enviado' ? 'bg-blue-100 text-blue-700' : 'bg-gray-200 text-gray-700'
                                            }`}>
                                                {pedido.estado}
                                            </span>
                                        </td>
                                        ...
                                        <td className="p-5 text-right flex justify-end items-center gap-4">
                                            ...
                                            >
                                                {pedido.estado === 'Borrador' ? 'Editar' : 'Ver'}
                                            </button>
                                ...
                                <tr>
                                    <td colSpan={6} className="p-10 text-center text-gray-400">
                                        No se encontraron pedidos de {tipoPedido}.
                                    </td>
                                </tr>
                            )}
                        </tbody>
