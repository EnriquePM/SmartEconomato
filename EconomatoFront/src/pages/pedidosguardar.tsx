...
const Pedidos = () => {
    ...
    // Filtramos usando los nombres de campos que vienen de Prisma/Servicio
                            ...
                            <div className="relative">
                                ...
                                >
                                    ...
                                    {catalogoProveedores.map((p: any) => (
                                        // Usamos id_proveedor que es el campo real de tu modelo Proveedor
                                        ...
                                        <td className="p-3 w-1/3 align-middle">
                                            ...
                                            >
                                                ...
                                                {catalogoProductos.map((p) => (
                                                    // p ya viene como ItemInventario del hook (con id, nombre, etc)
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                ...
                <div className="w-full md:w-auto">
                    <button onClick={() => { 
                        ...
                        setVista('formulario'); 
                    }} className="flex items-center justify-center gap-2 bg-black text-white px-6 py-3 rounded-pill font-bold hover:bg-gray-800 transition-all shadow-lg active:scale-95 w-full md:w-fit"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>NUEVO PEDIDO</button>
                </div>
            </div>
                            ...
                            {pedidosFiltrados && pedidosFiltrados.length > 0 ? (
                                pedidosFiltrados.map((pedido) => (
                                    <tr key={pedido.id_pedido} className="hover:bg-gray-50 transition group">
                                        <td className="p-5 font-mono text-xs text-gray-500">{pedido.id_pedido}</td>
                                        <td className="p-5 font-medium text-gray-900">{pedido.proveedor}</td>
                                        <td className="p-5 text-gray-600">{pedido.fecha_pedido ? new Date(pedido.fecha_pedido).toLocaleDateString() : '-'}</td>
                                        <td className="p-5 text-center">
                                            <span className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase ${
                                                pedido.estado === 'PENDIENTE' ? 'bg-blue-100 text-blue-700' : 
                                                pedido.estado === 'BORRADOR' ? 'bg-gray-100 text-gray-600' :
                                                'bg-green-100 text-green-700'
                                            }`}>
                                                {pedido.estado}
                                            </span>
                                        </td>
                                        <td className="p-5 text-right font-bold text-gray-800">{Number(pedido.total_estimado || 0).toFixed(2)} €</td>
                                        <td className="p-5 text-right flex justify-end items-center gap-4">
                                            <button 
                                                onClick={() => { 
                                                    // Aseguramos que cargamos las líneas si existen
                                                    setPedidoActual({...pedido, lineas: pedido.lineas || []}); 
                                                    setVista('formulario'); 
                                                }} 
