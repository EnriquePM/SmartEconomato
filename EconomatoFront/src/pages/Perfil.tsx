...
const Perfil = () => {
  ...
  const [usuario, setUsuario] = useState(() => {
    ...
    const authUser = dataUser ? JSON.parse(dataUser) : null;

    return {
      nombre: authUser?.nombre || "Usuario",
      apellidos: `${authUser?.apellido1 || ''} ${authUser?.apellido2 || ''}`.trim() || "",
      email: authUser?.email || "Sin email configurado",
      rol: authUser?.rol || "Alumno",
      avatar: sessionStorage.getItem("avatarUsuario") || chef1 
    };
  });
  ...
  const guardarCambios = () => {
    ...
    sessionStorage.setItem("avatarUsuario", avatarSeleccionado);
