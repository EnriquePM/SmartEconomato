...
import { PackageOpen, X, Search } from "lucide-react";
            ...
            <div className="px-8 py-6 bg-white overflow-y-auto">
              <div className={`p-8 rounded-[2.5rem] animate-fade-in-up border transition-all ${
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  />
                  <Input 
                    id="o-rec" type="text" label="Notas" placeholder="Añadir nota..."
                    value={lineaEnFoco.observaciones} 
                    onChange={(v) => actualizarValor(lineaEnFoco.id_referencia, 'observaciones', v)}
                  />
                </div>

                <div className=" py-6 border-t border-gray-100 flex justify-end gap-3 shrink-0 mt-8">
                  <Button variant="gris" className="flex-1" onClick={() => setLineaEnFoco(null)}>VOLVER AL LISTADO</Button>
                  <Button variant="primario" className="flex-[2]"  onClick={enviarDatos}>GUARDAR CAMBIOS</Button>
                </div>
              </div>
            </div>
          ) : (
